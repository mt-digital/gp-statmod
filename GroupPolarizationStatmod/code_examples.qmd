...
statistical power of $W=0.8$. We consider three effect sizes in this paper according to
the : "small", "medium", and "large" effect sizes traditionally assumed practically
meaningful across social sciences [@CohenBook1988]. I put this function in the `scripts` directory because to run the full analyses presented in the paper it's only necessary to run `calc_fdr_vs_significance` since it creates a column for the false positive rate and the estimated false detection rate calculated for each experimental design we studied.

This is reported for a variety of effect-detection thresholds, $\delta^*$, which the function name implies are "significance" values. When I first wrote this function
which are really effect-detection thresholds—so the code slightly misuses the term "significance", which typically refers only to a p-value based effect-detection threshold in the *null-hypothesis significance testing* approach. I updated the prose in the article to reflect this distinction, but left the code as-is. I again call the function with the defaults for illustration.
