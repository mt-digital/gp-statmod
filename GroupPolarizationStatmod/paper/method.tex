...
\begin{equation}\phantomsection\label{eq-random-variable-def}{ 
\omega_{i,t} \sim
\mathcal{N}(\mu_t, \sigma_t),
}\end{equation} with mean \(\mu_t\) and standard deviation \(\sigma_t\).
...
our method can be inverted to \emph{evaluate} experimental designs in
the pre-registration stage to avoid preventable replication failures
like we identify here.
...
ten journal articles in our corpus, which we label \(e\).
...
polarization? We simulate experimental measurements of group
...
\subsubsection{Example: Invalidating the ``Americans'' condition in
Moscovici and Zavalloni
(1969)}\label{example-invalidating-the-americans-condition-in-moscovici-and-zavalloni-1969}
...
The next step is to scale up the analysis we did manually in the example
above, including develop a systematic way to find, and not guess, the
set of null model parameters that

\begin{figure}

\centering{

\includegraphics[width=10.53125in,height=\textheight,keepaspectratio]{Figures/Model/SoftwareMap_Mashup.png}
...
it leaves unanswered questions. First, we would like to know how what
the chance was that a published detection of group polarization was
actually a false positive, assuming that participant opinions were
distributed according to the null model identified in the first part of
...
\(b=0.10\). The relationship between the units of analysis and the
estimated error rates is summarized in Table \textbf{?@tbl-error-rates}.

Statistical Error Rates and Units of Analysis
...
The rate of falsely rejecting the null hypothesis (\(d=0\)) for that
single test (used in aggregation).

All experiments in a study, \(s\)

Family-Wise

\(\alpha_s\)

The mean Type I Error Rate across all conditions in that specific study.
...
The mean Type I Error Rate across all conditions identified as plausibly
spurious.
...
\item
  ...
  the null model \(\nu_e\):
  \[\omega_{i,t} \sim \mathcal{N}(\mu, \sigma_t)\] where \(\mu\) is the
  constant latent mean and \(\sigma_t\) is the time-specific standard
  deviation derived from the mere agreement null model \(\nu_e\).
\item
...
\subsection{Analysis Outline and
Summary}\label{analysis-outline-and-summary}
\addcontentsline{toc}{section}{Analysis Outline and Summary}

%\phantomsection\label{refs}
%\begin{CSLReferences}{1}{0}
%\bibitem[\citeproctext]{ref-DeGroot1974}
%DeGroot, Morris H. 1974. {``{Reaching a Consensus}.''} \emph{Journal of
%the American Statistical Association} 69 (345): 118--21.
%
...
%
%\bibitem[\citeproctext]{ref-Moscovici1969}
%Moscovici, Serge, and Marisa Zavalloni. 1969. {``{The group as a
%polarizer of attitudes}.''} \emph{Journal of Personality and Social
%Psychology} 12 (2): 125--35.
%
%\bibitem[\citeproctext]{ref-Schkade2010}
%Schkade, David, Cass R. Sunstein, and Reid Hastie. 2010. {``{When
%deliberation produces extremism}.''} \emph{Critical Review} 22 (2):
%227--52. \url{https://doi.org/10.1080/08913811.2010.508634}.
%
%\end{CSLReferences}
%
%
