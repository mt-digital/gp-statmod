...
The first step is to find which replications of the 57 must be retracted 
due to non-identifiability. We establish non-identifiability by finding one
set of latent null-polarization opinion distributions where the mean doesn't change,
but appear polarized when measured on the ordinal scale used in the experiment.


To do
this we designed a web app that allows the user to try different latent
...
—latent mean and pre- and post-discussion 
variances that  loads experimental design and observation data
from a user-provided CSV file

ordinal measurements distort to look 
polarized. 
...
parameters $\nu$ that create binned (Equation~\ref{eq:ordinal-frequency})
