...
simulated average pre-discussion opinion definition (@eq-sim-ave-observed), $$
...
radicalization (CHECK THE PRESENTATION OF ALL THREE ABOVE) $$
...
that invalidated the replication, if one was found. We simulate data from a
single group polarization experiment by sampling participants' ordinal opinions
at pre-/post-discussion from the pre- and post-discussion opinion distributions
specified by the null model that invalidated the replication in that experiment,
$\nu_e$. If there are $N_e$ participants in experiment $e$ then our simulated
data sets contained $2N_e$ rows: two time step rows per participant. We recorded
$N_e$ used by each experiment in the web app as metadata along with data about
average opinions and opinion shifts for each experimental condition across the
corpus.

Group polarization is "detected" if the group polarization effect size meets or
exceeds a threshold value. We use Cohen's $d$ for the effect size, defined as
[@Liddell2018, 331] $$
...
$\nu$, when used in the context of calculating Cohen's $d$. Note $d=0$ always
for the true $\nu$.
...
There are many possible null models we could choose.

what significance threshold would have limited inverting this procedure to find
what significance value $d^*$ is necessary to achieve a low family-wise error
rate $\alpha_e = 0.05$ for experimental condition, $e$.

To do this we first calculate $\alpha_e(d^*)$ for a sufficiently wide range of
$d^*$. We then find the lowest $d^*$ for which $\alpha_e(d^*) \leq 0.05$.
