...
If an extremist opinion falls and it is measured with a Likert scale, will it make a noise?
...
\emph{Group polarization} is the name given the social phenomenon of 
...
condition $e$, written
\begin{equation}
  \alpha_e = \frac{1}{N_T} \sum_i^{N_T} \Theta_i,
\end{equation}
\noindent
where
\[
  \Theta_i = 
    ...
    \end{cases}
\]
and $N_T = 1000$ is the number of trials.
...
\end{equation}
\noindent
The grand mean across all studies for the corpus is 
\begin{equation}
  \alpha_\mathrm{all} = \frac{1}{|E|} \sum_{e \in E} \alpha_e.
\end{equation}
With $\alpha$ calculated, we calculate the false discovery rate using
Equation~\ref{eq:fdr}.



We invert this procedure to find what significance value $d^*$ is necessary
to achieve a low family-wise error rate $\alpha_e = 0.05$ for experimental
condition, $e$. To do this we first calculate $\alpha_e(d^*)$ for a
...
opinions-ordinal measurement experimental design. We then analyze family-wise
error and false detection rates for each experimental condition and aggregated 
up to the study level and across all studies and experimental conditions. 
Family-wise error rates varied from a minimum of 0.097 for the
\texttt{Feminists-Experimental} condition from Myers (1975)~\cite{Myers1975a}
and a maximum of 0.67 for the \texttt{COSprings-CivilUnions} condition from
Schkade, Sunstein, and Hastie (2010)~\cite{Schkade2010}. The median family-wise error
rate is 0.28 for Krizan and Baron's experimental condition
\texttt{NoOutgroupScenario1}. We close by inverting our procedure of calculating
family-wise error rates to calculating what $d^*$ would be required in each of the 54
experimental conditions to reduce the family-wise error rate to $\alpha = 0.05$.


\subsection{Nearly all group polarization detections are plausibly false}
...
Of the remaining 57, we identified plausible $\beta$ that generate spurious
...
simulated distributions were highly polarized. Two of these
simulations of experimental conditions from Schkade, et al., (2010) because
...
rejected candidate $\beta$ was in a question about a ``bad teacher'' in Myers
(1975)~\cite{Myers1975a}, where lower values signify increasingly 
worse impressions of the teacher and higher values signify increasingly good
impressions. It is unlikely that any participants would rate a ``bad teacher''
as good, so we excluded this from our analysis as well. This exclusion may be
overly generous since Myers (1975) did not provide details beyond mean values
and opinion shifts, and the article was published before the importance
of open data best practices was recognized.

\clearpage

\begin{table}[ht]
\rotatebox{90} {
  \input{tex/quantile_table.tex}
}
\end{table}

\subsection{Half of experiments exceed a 75\% false discovery rate at ``medium'' significance}

The $N\to\infty$ model enabled us to identify pre- and post-deliberation latent
opinion distribution parameters that caused spurious group polarization, denoted
$\beta_e$ for experimental condition $e$. Using these $\beta$ we drew 
$N_\mathrm{emp}$ samples from the pre- and post-deliberation latent opinion 
distributions, then binned them (Section~\ref{subsec:false-detections}) 
to simulate participants reporting their pre- and post-deliberation opinions. 
To estimate optimistic family-wise error and false detection rates, we fit an
ordered probit model to the simulated observations and obtained estimates of
the group polarization effect size, $d$ (Equation~\ref{eq:cohens}).
If the effect size was greater than a significance value, $d > d^*$, then there
was a detection $D$ of group polarization, even though by design the simulated latent mean
group opinion was constant, i.e., there in fact was no effect, written $\neg E$.
The family-wise error rate is the calculation of $\Pr(D|\neg E)$ as the frequency with which
$d > d^*$ out of 1000 simulation trials. The false detection rate is the family-wise
error rate scaled using the base rate of effects and the statistical power
(Equation~\ref{eq:fdr}). We test three difference significance values,
$d = 0.2,~0.5$ and $0.8$, corresponding to what Cohen prescribed to use for
``low'', ``medium'', and ``high'' significance, respectively.



We calculated the false detection rate across experimental conditions where
we identified parameters that generate spurious group polarization in the 
large-$N$ model (Figure~\ref{fig:fdr_experiments}). We then aggregated over
experimental conditions within each study to understand the false detection rate
across the ten studies (Figure~\ref{fig:fdr_studies}). Across the identified
spurious conditions, we calculated a mean false detection rate of 0.74 with
a significance level of $d^* = 0.5$. We also
examined the mean false detection rates across studies if we set a highly
optimistic estimate of the family-wise error rate, setting $\alpha = 0.05$. 
This only reduces the false discovery rate to 0.71, but does slightly change the
rankings of which study had the highest mean false discovery rates, with 
Schkade, et al., (2010) going from highest mean false discovery rate to 
second-lowest false discovery rate. The minimum false discovery rate in a study
was Myers (1975), with a false discovery rate of 0.58, meaning that probability
a group polarization detection in the best-designed study is greater than one in
two.
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=1.1\textwidth]{Figures/Results/fwer_fdr_synthesis.pdf}
...
\begin{figure}
  \caption{\textbf{Significance value, $d^*$, to limit family-wise error 
  rate of 0.05 for each identified study.} \mt{Does this make sense? Order by the
rankings in fdr\_vs\_experiment plots to better understand/compare.}}
  \centering
    \includegraphics[width=0.75\textwidth]{Figures/Results/sigval_for_low_fwer.pdf}
