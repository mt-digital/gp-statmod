...
\usepackage{etoolbox}
\makeatletter
\title{Decades of ``group polarization'' replications may be undermined by high false discovery rates}
...
group polarization experiments seeded with data from the studies
themselves.  We also use our model to develop experimental designs that
achieve an acceptable false discovery rate.  Much of group polarization
research may be unreliable, but this analysis can help change that. More
broadly, this work demonstrates one way that replication success alone
...
An opinion is never measured directly, only behaviors meant to indicate one's opinion.
For example, only the location of a mark that a participant makes is measured, not
the complex neural activity that somehow aggregates and expresses some latent
representation. Because so much neural activity must be integrated to yield an opinion,
it is most reasonable to assume opinions are continuous. We call these 
\textit{latent opinions} because they are unobservable. In contrast, ordinal-valued
\text{observed opinions} are the value participants report before and after group
