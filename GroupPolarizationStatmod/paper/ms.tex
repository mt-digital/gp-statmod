...
\begin{abstract}
  ...
  that induces false positives for each succeeding ``replication.'' Here we show that
  replicated detections of group polarization are often plausibly false due to
  treating ordinal measurements of opinions, e.g., on a Likert-type scale, as drawn
  directly from an underlying continuous latent opinion distribution, which masks
  extreme latent opinions. Our intensive review of sixty experimental conditions
  across ten foundational group polarization papers revealed that 95\% of the
  conditions generated plausibly false positive results. To show this, we developed a
