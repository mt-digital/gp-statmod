...
polarization is a scientific law—but Roger Brown (1986, Ch. 5) wrote that group polarization ``does not occur with every group'' and that ``the
effect is not large,'' conflicting with the standard definition that lawlike
behavior means certain observations will obtain given that specific assumptions
are satisfied~\cite{Cartwright1999}. Given the prominence of group polarization 
theory, we decided to analyze whether its hundreds of claimed replications
may instead be measurement artifacts using a formal model and numerical simulations of
...
\centering
\begin{tabular}{p{1in}p{3.5in}p{2in}c}
\toprule
