...
HDIofMCMC = function( sampleVec , credMass=0.95 ) {
  ...
  sampleVec = sampleVec[is.finite(sampleVec)] # omit Inf , NaN
...
plotPost = function( paramSampleVec , cenTend=c("mode","median","mean")[1] , 
  ...
  paramSampleVec = paramSampleVec[ is.finite(paramSampleVec) ]
  ...
  ropeCol = "darkred"
  if ( is.null(breaks) ) {
    if ( max(paramSampleVec) > min(paramSampleVec) & HDI[2] > HDI[1] ) {
