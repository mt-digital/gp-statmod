...
\newcommand{\nullmodel}{\mathcal{M}_0}


<!-- Begin main document -->
# Introduction

This document supports my article with Paul Smaldino, *Null Fits? Must Omit!* It follows the structure of the article's _Method_ and _Analysis_ sections, providing a tutorial explanation, first, of the code used to model group polarization experiments and opinion measurement; and, second, how the code works to summarize, analyze, and visualize simulation results. 

Our article describes how we analyzed simulations of group polarization experiments to show that ~95% of experimental replications of the group polarization effect are invalid due to non-identifiability from measurement artifacts across ten journal articles that comprised our curated corpus. There were a total of 57 independent experimental conditions across these studies. 

We furter estimated at least a 70% chance that any published replication of group polarization is, in fact, a false detection. 

We also show how this simulation system can be used constructively to specify what effect size should be used as a detection threshold to limit FPR to 0.05. For more details of the motivation, theory, and results of our analysis, please see our paper.


**You can read this documentation online, hosted by GitHub:** https://mt-digital.github.io/gp-statmod

**Or, you can read this document and run the code examples yourself.** To follow along and run code on your own, please clone the `gp-statmod` directory:
...
Change directories to go to the `gp-statmod` directory where this document lives:
...
polarization model, $\nullmodel = (\mu, \sigma_\mathrm{pre}, \sigma_\mathrm{post})$.
...
1.  **Calculate false positive/detection rates for the simulated null polarization
    ...
    standard deviation of latent opinions, but a constant mean latent opinion. Here, then, we calculate the false positive rate, $\alpha_e$ as a function of the effect-detection threshold, $d^*$, i.e., 
    $$
      \alpha_e(d^*) = \Pr\!\left( \hat{d} \ge d^{*} \mid \mathcal{M}_{0} \right).
    $$ {#eq-false-positive-rate}
    In the study, we calculate the right-hand side as the frequency of the 1000 trials where $\hat{d} \ge d^*$ for simulations of experiment, $e$. 
2.  **Find an effect–detection threshold**, $d^*(\alpha)$, such that, under the null
    ...
    \$d\^\*(\\alpha)\$ is some target false positive rate, $\alpha^* = 0.05$, for example: 
    $$
...
experimental data from a mere agreement null-polarization model. Then we can calculate how frequently an effect size was inferred that exceeded the effect-detection threshold.
...
In the second step of our generative model analysis I calculate the solution to
the problem in @eq-threshold-spec, finding the effect-detection threshold, $d^*$, that achieves a target false positive rate, $\alpha^* = 0.05$. We need an updated dataset of Type I error rates calculated with a finer-grained set of candidate effect-detection thresholds, which we just call $d$ if they're candidates, than the simple three, 0.2, 0.5, and 0.8, used in the previous section. 


```{r}
# The fwer is equivalently Type I error—written before I understood the difference,
# not worth changing the name for this long-running project before it's done.
det_thresh_tbl <- sigval_for_low_fwer(fdr_over_thresholds, target_fwer = 0.05)

print(det_thresh_tbl)
```
